...
if response.status_code == 200:
    with open(f'Data/{year}년 제{No}회 신기술(NET) 인증기술 공고.pdf', 'wb') as f:
...
# PDF 파일 열기
with pdfplumber.open(f'Data/{year}년 제{No}회 신기술(NET) 인증기술 공고.pdf') as pdf:
    # 추출한 데이터프레임을 저장할 리스트
    all_tables = []
    ...
    if all_tables:
        ...
        df4.to_csv(f'Data/009_NET_{formatted_date}.csv', index=False, sep=',')
    else:
